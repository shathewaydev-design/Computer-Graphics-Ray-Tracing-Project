/****************************************************
 * 2016-2025 Eric Bachmann and Mike Zmuda and Norm Krumpe
 * All Rights Reserved.
 * NOTICE:
...
double normalizeDegrees(double degrees) {
	...
	return degrees - (360 * glm::floor(degrees / 360));
...
double normalizeRadians(double rads) {
	...
	return rads - ((2 * PI) * glm::floor(rads / (2 * PI)));
}
 ...
 * Programming constraint: Do not use glm::degrees.
 * @param	rads	The radians.
 * @return	Degrees.
 */
...
/**
 * @fn	bool isTriangle(double a, double b, double c)
 * @brief	Determines whether three sides can form a triangle.
 * @param	a length of first side.
 * @param	b length of second side.
 * @param	c length of third side.
 * @return	true if a, b, c can form a triangle, and false otherwise
 * @test	isTriangle(3, 5, 6) --> true
 * @test	isTriangle(-3, 5, 6) --> false
 * @test	isTriangle(8, 0, 9) --> false
 * @test	isTriangle(3, 5, 100) --> false
 * @test	isTriangle(8, 3, 5) --> false
 */
 ...
 * @param	b length of second side.
 * @param	c length of third side.
 * @return	Area of triangle. Returns -1.0 if the sides do not form a triangle
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	...
	double sideC = distanceBetween(x3, y3, x1, y1);
...
void pointOnUnitCircle(double angleRads, double& x, double& y) {
	...
	if (approximatelyZero(x)) {

		x = 0.0;
	
	}

	if (approximatelyZero(y)) {
...
double directionInRadians(const dvec2& targetPt) {
	/* CSE 386 - todo  */
	return directionInRadians(dvec2(0.0, 0.0), targetPt);
}
...
double map(double x, double fromLo, double fromHi, double toLo, double toHi) {
	/* CSE 386 - todo  */
	double firstTotal = fromHi - fromLo;
	double secondTotal = toHi - toLo;
	double ratio = (x - fromLo) / firstTotal;
	double newRatio = ratio * secondTotal;
	return toLo + newRatio;
}

// Helper method that calculates the discriminant of a quadratic equation
...
vector<double> quadratic(double A, double B, double C) {
	/* CSE 386 - todo  */
	vector<double> result;	// put only the roots in here, low to high
	double disc = discriminant(A, B, C);

	if (disc > 0) {
		double root1 = ((-1 * B) + glm::sqrt(disc)) / (2 * A);
		double root2 = ((-1 * B) - glm::sqrt(disc)) / (2 * A);
 ...
 *			The length of the roots array does not change, but its
 *			contents may change. If the quadratic equation has no
 *			roots, then 0 is returned and the contents of the roots array
 *			do not need to be set. If the quadratic equation has one
 *			root, then 1 is returned and the first element of the roots
 *			array is set to that root. If the quadratic equation has two
 *			roots, then 2 is returned and the two elements of the array
 *			are set to those roots, in ASCENDING order.
 * 
 ...
 * @test	quadratic(1 ,0, 0, ary) --> returns 1 and fills in ary[0] with: 0
...
int quadratic(double A, double B, double C, double roots[2]) {
	...
	vector<double> rootVals = quadratic(A, B, C);

	int rootCnt = rootVals.size();
...
dvec3 myNormalize(const dvec3& V) {
	...
	double normFact = glm::sqrt((V.x * V.x) + (V.y * V.y) + (V.z * V.z));
	...
	// ASK IF WE ARE RETURNING A NEW VECTOR!!!
	//return V;

	return dvec3(V.x * normFact, V.y * normFact, V.z * normFact);
...
double cosBetween(const dvec2& v1, const dvec2& v2) {
	/* CSE 386 - todo  */
	return glm::dot(v1, v2) / (glm::sqrt((v1.x * v1.x) + (v1.y * v1.y)) 
		* glm::sqrt((v2.x * v2.x) + (v2.y * v2.y)));
}
...
double cosBetween(const dvec3& v1, const dvec3& v2) {
	/* CSE 386 - todo  */
	return glm::dot(v1, v2) / (glm::sqrt((v1.x * v1.x) + (v1.y * v1.y) + (v1.z * v1.z))
		* glm::sqrt((v2.x * v2.x) + (v2.y * v2.y) + (v2.z * v2.z)));
}
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	...
	double triArea = areaOfParallelogram((pt2 - pt1), (pt3 - pt1));
	return triArea / 2;
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	...
	return (pt2 - pt1) / (glm::length(pt2 - pt1));
}
